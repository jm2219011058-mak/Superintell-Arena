...
# ═══════════════════════════════════════════════════════════════
# Debate Mode — Dialectical Debate Simulation Extension
# ═══════════════════════════════════════════════════════════════


class DebateActionType(str, Enum):
    """Debate action types (replaces OASIS social media actions for debate mode)."""
...
class DebatePhase(str, Enum):
    """Debate phases (replaces OASIS time-based simulation for debate mode)."""
    OPENING = "opening"           # Each agent presents initial stance
    CROSS_EXAM = "cross_exam"     # Agents challenge each other directly
    SYNTHESIS = "synthesis"       # Agents produce cross-framework insights


@dataclass
class DebateAgentAction:
    """Single action taken by an agent during debate."""
    round_num: int
    phase: DebatePhase
    agent_id: str
...
class DebateRound:
    ...
    round_num: int
    phase: DebatePhase
    actions: List[DebateAgentAction] = field(default_factory=list)
    phase_summary: str = ""
...
@dataclass
class DebateAgentProfile:
    """
    Agent profile for debate. Extends the arena-backend's agent profile pattern.

    Base fields mirror MiroFish: agent_id, name, bio, persona.
    ...
    # Graph origin (from Zep GraphRAG)
...
class DebateConfig:
    """
    Debate simulation configuration. Extends the arena-backend's simulation config pattern.
    """
    topic: str
    num_rounds: int = 3
    ...
    agents: List[DebateAgentProfile] = field(default_factory=list)
...
# Debate agent system prompt template
...
class DebateSimulationRunner:
    ...
    Extends the arena-backend's SimulationRunner with:
    ...
    def __init__(self, config: DebateConfig, llm_client=None,
        ...
        self.state = RunnerStatus.IDLE
        self.rounds: List[DebateRound] = []
        self.action_log: List[DebateAgentAction] = []
        self._callbacks: Dict[str, list] = {}

    # --- State Machine ---

    def _transition(self, new_state: RunnerStatus):
        valid = {
            RunnerStatus.IDLE: [RunnerStatus.STARTING],
            RunnerStatus.STARTING: [RunnerStatus.RUNNING, RunnerStatus.FAILED],
            RunnerStatus.RUNNING: [RunnerStatus.PAUSED, RunnerStatus.STOPPING,
                                   RunnerStatus.COMPLETED, RunnerStatus.FAILED],
            RunnerStatus.PAUSED: [RunnerStatus.RUNNING, RunnerStatus.STOPPING],
            RunnerStatus.STOPPING: [RunnerStatus.STOPPED],
        }
    ...
    async def run(self) -> List[DebateRound]:
        ...
        """
        self._transition(RunnerStatus.STARTING)

        try:
            self._transition(RunnerStatus.RUNNING)

            for round_num, phase in enumerate(self.config.phases):
                if self.state != RunnerStatus.RUNNING:
                    break

                debate_round = await self._run_round(round_num, phase)
                self.rounds.append(debate_round)

                # Memory writeback after each round
            ...
            self._transition(RunnerStatus.COMPLETED)
            return self.rounds

        except Exception as e:
            self._transition(RunnerStatus.FAILED)
            raise
    ...
    def _parse_agent_response(self, agent: DebateAgentProfile, round_num: int,
                               phase: DebatePhase, response: str) -> DebateAgentAction:
        """Parse LLM response into structured DebateAgentAction."""
        stance = "neutral"
    ...
    async def _writeback_round(self, debate_round: DebateRound):
        ...
        and updates agent entity nodes with new beliefs/concessions.
        """
    ...
    def _action_to_description(self, action: DebateAgentAction) -> str:
        """Convert a DebateAgentAction to natural language for Zep memory."""
    ...
    def export_log(self) -> List[dict]:
        """Export action log as JSON-serializable list."""
        return [
            {
