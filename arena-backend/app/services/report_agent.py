...
# ═══════════════════════════════════════════════════════════════
# Debate Mode — Dialectical Debate Report Extension
# ═══════════════════════════════════════════════════════════════
...
class StancePosition:
    ...
    concessions_made: List[str] = _field(default_factory=list)
...
class InsightCard:
    ...
    category: str    # e.g. "核心涌现洞察", "关键分歧", "可行动预测", "反脆弱处方"
    title: str
    content: str
...
@_dataclass
class DebateReport:
    """Complete debate report output. Rendered by the frontend Report screen."""
    topic: str
    agent_count: int
    round_count: int
...
class DebateZepToolsService:
    """
    Wraps the arena-backend's ZepToolsService with debate-specific queries.
    ...
    async def insight_forge(self, query: str, topic: str) -> dict:
        ...
        Decomposes query into sub-queries, searches graph + vector, merges results.
        """
    ...
    async def cross_framework_search(self, framework_a: str, framework_b: str) -> dict:
        """
        Search for edges/connections between two frameworks.
    ...
    async def tension_search(self, topic: str) -> dict:
        """
        Find all CONTRADICTS and CHALLENGES relationships for a topic.
...
# Debate report system prompt
DEBATE_REPORT_SYSTEM_PROMPT = """You are the DebateReportAgent for Superintell Arena.
...
class DebateReportAgent:
    ...
    Extends the arena-backend's ReportAgent:
    ...
    async def generate_report(self, topic: str, rounds: list,
        ...
        report.generation_time_ms = int((_time.time() - start) * 1000)
        return report

    async def _react_loop(self, topic: str, rounds: list, agents: list) -> DebateReport:
        """
        Full ReACT loop for report generation.
        Mirrors the arena-backend's ReportAgent._generate_section_react() pattern.
        """
    ...
    def _build_demo_report(self, topic: str, rounds: list, agents: list) -> DebateReport:
        ...
        insights = [
            ...
            InsightCard("可行动预测", "三个投资级机会",
                "1. 信任协议平台 2. 双轨组织架构 3. 自动化多元模型检验系统",
